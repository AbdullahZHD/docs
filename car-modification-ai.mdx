...
Ensure you're developing in [localhost](http://localhost) mode for proper testing and development workflow.

\</Note\>

### How to change to [localhost](http://localhost) mode
...
\<Steps\>
  ...
  \<Step title="Save and test"\>
    ...
    Save the changes and test your app installation to ensure [localhost](http://localhost) mode is working.
...
Make sure to get your OpenAI API key from [[platform.openai.com](http://platform.openai.com)](https://platform.openai.com/api-keys). You'll need access to gpt-image-1 model for the best car image analysis.
...
export function downloadCanvasAsImage(canvas: HTMLCanvasElement, filename: string) {
  ...
  [link.download](http://link.download) = filename;
...
export function createMaskFromCanvas(canvas: HTMLCanvasElement): string {
  ...
  const data = [imageData.data](http://imageData.data);
...
export default function CarModifier({ experienceId }: CarModifierProps) {
  ...
  const handleFileUpload = (e: React.ChangeEvent\<HTMLInputElement\>) =\> {
    ...
    const file = [e.target](http://e.target).files?.[0];
              ...
              \<\>
                ...
                \<div className="bg-gray-50 rounded-lg p-4"\>
                  ...
                  \<input
                    ...
                    onChange={(e) =\> canvasPainter.setBrushSize(Number([e.target](http://e.target).value))}
          ...
          \<div className="space-y-4"\>
            ...
            \<div\>
              ...
              \<textarea
                ...
                onChange={(e) =\> carModification.setPrompt([e.target](http://e.target).value)}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (\![result.data](http://result.data)?.[0]?.b64_json) {
    ...
    return NextResponse.json({
      ...
      modifiedImage: [result.data](http://result.data)[0].b64_json,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log('📁 Uploading file:', { name: [file.name](http://file.name), size: file.size, type: file.type });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const companyId = [experienceResult.experience.company.id](http://experienceResult.experience.company.id);
    ...
    if (attachments && attachments.length \> 0) {
      ...
      postInput.attachments = [attachments.map](http://attachments.map)((id: string) =\> ({
    ...
    console.log('✅ Post created successfully with ID:', [post.id](http://post.id));

    return NextResponse.json({

      success: true,

      postId: [post.id](http://post.id),

      forumLink: [forumResult.createForum.link](http://forumResult.createForum.link),
    ...
    console.error('Error details:', {
      ...
      name: error instanceof Error ? [error.name](http://error.name) : 'Unknown',
...
\<Steps\>
  ...
  \<Step title="Create a new project on Vercel"\>
    ...
    Go to [[vercel.com](http://vercel.com)](https://vercel.com) and click "New Project"
